...
public:
    ...
    RobotAction GetRobotAction(const RobotBasicState& ro) override {
        ...
        cmd_vel.head(3) = ro.cmd_vel_normlized.head(3).cwiseProduct(max_cmd_vel_);
        cmd_vel[3] = ro.cmd_vel_normlized[3];  // 第4维：模式命令

        // 调试：打印接收到的命令
    if (run_cnt_ % 200 == 0) {
        std::cout << "=== 策略运行器调试信息 ===" << std::endl;
        std::cout << "模式命令: " << ro.cmd_vel_normlized[3] << std::endl;
        std::cout << "线速度命令: " << ro.cmd_vel_normlized.head(2).transpose() << std::endl;
        std::cout << "角速度命令: " << ro.cmd_vel_normlized[2] << std::endl;
        std::cout << "投影重力: " << projected_gravity.transpose() << std::endl;
        std::cout << "=========================" << std::endl;
    }
        for (int i = 0; i < act_dim_; ++i){
        ...
        //     // 第一次调用，直接使用原始动作
        //     filtered_actions_ = action;
        //     is_filter_initialized_ = true;
        // } else {
        //     // 应用低通滤波：filtered = alpha * current + (1-alpha) * previous
        //     filtered_actions_ = action_filter_alpha_ * action + (1.0f - action_filter_alpha_) * filtered_actions_;
        // }
        
        // // 使用平滑后的动作
        // last_action = filtered_actions_;  // 注意：这里使用filtered_actions_而不是原始action
        
        // for (int i = 0; i < act_dim_; ++i){
        //     tmp_action(i) = filtered_actions_(policy2robot_idx[i]);  // 使用平滑后的动作
