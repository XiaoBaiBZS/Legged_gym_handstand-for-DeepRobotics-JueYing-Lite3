...
private:
    ...
    // std::mutex mtx_;  //  保护 usr_cmd_ 和 msfb_
     ...
     // 添加模式切换状态
    bool mode_toggle_state_ = false;  // false=0, true=1
...
public:
    ...
    void Run(){
        ...
        double comand_time_record = GetCurrentTimeStamp();
                ...
                switch(msfb_.current_state) {
                    ...
                    case RobotMotionState::RLControlMode:
                        ...
                        if(input == 'x') {
                            ...
                            // 设置模式命令：0=自由模式，1=站立模式
                            usr_cmd_.mode_command = mode_toggle_state_ ? 1.0f : 0.0f;
                            
                            // 添加调试信息
                            std::cout << "[键盘] 模式切换: " << (mode_toggle_state_ ? "手倒立模式" : "站立模式") 
                                    << " (命令值: " << usr_cmd_.mode_command << ")" << std::endl;
                            
                            comand_time_record = current_time;
